	...
	public void actionPerformed(ActionEvent e) {
		Display.getDefault().asyncExec(new Runnable() {
			...
			public void takeScreenshot(Display display, Composite shell) {
				...
				// the Eclipse window, so parent == the display which is null),
				// then take the screenshot using the shell coordinates
				if (shell.getParent() == null) {
					...
					// parent shell back to takeScreenshot. Recursion should be
					// minimal as there will only ever be a small number of
					// Eclipse windows open
