...
public class PartUsageMonitor implements UsageMonitor {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see ca.ubc.cs.commandrecommender.usagedata.gathering.UsageMonitor#register(ca.ubc.cs.commandrecommender.usagedata.gathering.UsageDataService)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see ca.ubc.cs.commandrecommender.usagedata.gathering.UsageMonitor#deregister()
	 */
...
//		});
	...
	protected void recordEvent(String event, IWorkbenchWindow window) {
		...
		usageDataService.recordEvent(event, WORKBENCH, EMPTY_STRING, WORKBENCH_BUNDLE_ID, "0");
	}

	protected void recordEvent(String event, IPerspectiveDescriptor perspective) {
		String id = perspective.getId();
		usageDataService.recordEvent(event, PERSPECTIVE, id, perspectiveToBundleIdMapper.getBundleId(id), "0");
	}
	
	private void recordEvent(String event, IWorkbenchPart part) {
		IWorkbenchPartSite site = part.getSite();
		usageDataService.recordEvent(event, getKind(site), site.getId(), site.getPluginId(), "0");
	}
